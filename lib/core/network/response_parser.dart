...
import 'package:steel_soul/core/network/api_response.dart';
import 'package:steel_soul/core/network/exception.dart';
    ...
    try {
      ...
      if (response.containsKey('message')) {
        ...
        if (message is String) {
          // Let the caller's parser decide how to handle string messages
          final result = parser(response);
          return ApiResponse.success(result);
        }
        ...
        final status = msgObj['status'] is int ? msgObj['status'] as int : 200;
        if (status == 200) {
          final result = parser(response);
          return ApiResponse.success(result);
        ...
        // Handle empty response as success (common for action-based APIs)
        final result = parser(response);
        return ApiResponse.success(result);
      }
